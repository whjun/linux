			...
			<tbody id="list">
				<?php foreach ($list['data'] as $key => $value): ?>
					<tr class="text-c" value="<?= $value['brand_id']?>">
						...
						<td class="f-14 product-brand-manage"><a style="text-decoration:none" href="http://localhost/TrainingOne/laravel/public/index.php/brand_update?brand_id=<?= $value['brand_id']?>" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="del" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
					</tr>
				<?php endforeach ?>		
			</tbody>
		</table>
    	...
    	$.ajax({
    		...
    		success:function(res){
    			...
    			$.each(res.data,function(i,v){
    			    var tr = $("<tr class='text-c' value="+v.brand_id+"></tr>");
    			    tr.append("<td><input name='' type='checkbox' value=''></td>");
    			    tr.append("<td>"+v.brand_id+"</td>");
    			    tr.append("<td><input type='text' class='input-text text-c' value="+v.brand_order+"></td>");
    			    tr.append("<td><img src='../../storage/app/"+v.brand_logo+"' style='width: 40px;height: 40px\'></td>");
    			    tr.append("<td class='text-l'>"+v.brand_name+"</td>");
    			    tr.append("<td class='text-l'>"+v.brand_content+"</td>");
    			    tr.append("<td class='f-14 product-brand-manage'><a style='text-decoration:none' href='http://www.wangmingzhan.top/laravel/public/index.php/brand_update?brand_id="+v.brand_id+"' title='编辑'><i class='Hui-iconfont'>&#xe6df;</i></a> <a style='text-decoration:none' class='del' href='javascript:;' title='删除'><i class='Hui-iconfont'>&#xe6e2;</i></a></td>");
                    $("#list").append(tr);
                })
                $("#page").html("page");
    		}
